...
int main(void)
{
  ...
  for (;;) {
		 ...
		 OCR1A=mapVal(potVal,0,1023,90,640);
		
		 valToShow = mapVal(potVal,0,1023,0,100);
		 smoothFilter[i] = (int) valToShow;
		 i++;
		...
		if(i==3)
		{
			...
			for(int j=0;j<=2;j++)
			{
				average=average + smoothFilter[j]/3;
			}
			 sprintf(valueForLcd,"PWM=%d %%  ",average);
